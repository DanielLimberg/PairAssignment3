...
```{r, echo=FALSE, message=FALSE, include=FALSE, cache=FALSE}
summary(M5 <- lm(DAX.Close ~ GDPq.gr + L.unempl + L.cons.spend + L.ECB.MRO.ch + L.ECB.dep.ch, data = DEU))
summary(M6 <- lm(NIK.Close ~ GDPq.gr + L.unempl + L.cons.spend, data = JPN))
summary(M7 <- lm(CAC.Close ~ GDPq.gr + L.unempl + L.cons.spend + L.ECB.MRO.ch + L.ECB.dep.ch, data = FRA))
summary(M8 <- lm(FTSE.Close ~ GDPq.gr + L.unempl + L.cons.spend, data = GBR))
```
